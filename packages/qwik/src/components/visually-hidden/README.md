...
```tsx
import { VisuallyHidden } from '@frame-ui/qwik';
```
...
`VisuallyHidden` is a utility component. This section outlines the available props for its `VisuallyHidden.Root` subcomponent, enabling you to customize its behavior and appearance.
...
The following examples demonstrate the key use cases for the `VisuallyHidden` component, showing how it can be used to improve accessibility by making content available to screen readers while keeping it hidden from the visual interface.
...
By default, the `VisuallyHidden.Root` subcomponent renders a sensible HTML element, which is a `<span>`. For a complete overview of the default rendered element, please refer to the [Rendered elements](#rendered-elements) section.

You can customize the underlying HTML element rendered by `VisuallyHidden.Root`, or even compose it with another component from your library or your own custom Qwik components, by using the `render$` prop. This prop provides immense flexibility, allowing you to:

- Replace the default HTML tag (e.g., changing the `VisuallyHidden.Root` from a `<span>` to an `<h2>` as shown in the example, which is useful for an accessible, yet hidden, section title).
...
import { VisuallyHidden, Text } from '@frame-ui/qwik';
